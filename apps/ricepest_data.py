...
from apps.data_operation import save_to_postgis, load_from_postgis, load_ann_model_from_postgis, load_dataset_list, load_all_ann_model_from_postgis, load_bph_model_data


class RicePestDataLayers(HydraHeadApp):
    ...
    def run(self):
        ...
        def select_dataset_from_database(selected_dataset):
            return load_bph_model_data(selected_dataset)
        ...
        with st.sidebar:
            ...
            st.subheader('Rice Pest Outbreak')
            ...
            if selected_dataset != '-':
                bph_df = select_dataset_from_database(selected_dataset)
                #bph_df = bph_df.dropna(axis=1)
                selected_data_layer = st.multiselect("Select data layer to view",['Brown Plant Hopper','Mirid Bug','Weather','Rice Cultivated Area'])
                bph_years_list = pd.DatetimeIndex(bph_df['date']).year.unique()
                bph_year_options = st.multiselect("Select year of data", bph_years_list.values, bph_years_list.values[0])  
 

        st.subheader("Brown Plant Hopper Monitoring")
        row1_1, row1_2 = st.columns((2,5))
        st.write("View time-series brown plant hopper-related GIS data from database.")
        ...
        if selected_dataset != '-':
            if selected_data_layer:
                ...
                for x in selected_data_layer:
                    if x == 'Brown Plant Hopper':
                        selected_columns.append('bph')
                        chart_columns.append('bph')
                    if x == 'Mirid Bug':
                        selected_columns.append('mirid_bug')
                        chart_columns.append('mirid_bug')
                    if x == 'Weather':
                        selected_columns = selected_columns + ['mint', 'maxt', 'temp', 'dew', 'humidity', 'wspd', 'wdir', 'precip']
                        chart_columns = chart_columns + ['mint', 'maxt', 'temp', 'dew', 'humidity', 'wspd', 'wdir', 'precip']
                ...
                if selected_data_layer != '-' and len(bph_year_options) > 0:
                    selected_layer_df = bph_df[selected_columns]
                buff_set = pd.DataFrame()
                for yr in bph_year_options:
                ...
                with st.expander("View all BPH & Mirid Bug data chart", expanded=False):
                    for yr in bph_year_options:
                        ...
                        #lc = alt.Chart(chart_data.melt('date', var_name='bph', value_name='value')).mark_area().encode(x='date', y='value', color='bph',opacity=alt.condition(selection, alt.value(1), alt.value(0.2))).add_selection(selection)
                        #st.altair_chart(lc, use_container_width=True)
                        #print(chart_data)
                        dmelt = chart_data.melt('date', var_name='bph vs mirid bug', value_name='value')
                        #print(dmelt)
                        lc = alt.Chart(dmelt).mark_bar().encode(
                                x='date', y='value', color='bph vs mirid bug', tooltip=['date','value'])
                        st.altair_chart(lc, use_container_width=True)
