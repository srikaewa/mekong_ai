...
from apps.data_operation import save_to_postgis, load_from_postgis, load_ann_model_from_postgis, load_dataset_list, load_all_ann_model_from_postgis, load_flood_model_data


class FloodDataLayers(HydraHeadApp):
    ...
    def run(self):
        ...
        def select_dataset_from_database(selected_dataset):
            return load_flood_model_data(selected_dataset)
        ...
        with st.sidebar:
            ...
            st.subheader('Flood - Natural Disaster')
            ...
            if selected_dataset != '-':
                flood_df = select_dataset_from_database(selected_dataset)
                #flood_df = flood_df.dropna(axis=1)
                ...
                flood_year_options = st.multiselect("Select year of data", flood_years_list.values, flood_years_list.values[0])  
 

        st.subheader("Flood Monitoring")
        row1_1, row1_2 = st.columns((2,5))
        st.write("View time-series flood GIS data from database.")
            ...
            if selected_data_layer:
                ...
                for x in selected_data_layer:
                    if x == 'Flood Area':
                        ...
                        chart_columns.append('flood')
                    if x == 'Weather':
                        selected_columns = selected_columns + ['humidity', 'max_precip', 'mean_precip',	'avg_ssm', 'dem']
                        wchart_columns = ['date', 'humidity', 'max_precip', 'mean_precip',	'avg_ssm', 'dem']
                    """
                ...
                if selected_data_layer != '-' and len(flood_year_options) > 0:
                ...
                with st.expander("View all flood & weather data chart", expanded=False):
                    st.write("all chart")
                    for yr in flood_year_options:
                        for x in selected_data_layer:
                            if x == 'Flood Area':
                                ...
                                print(chart_data)
                                dmelt = chart_data.melt('date', var_name='flood', value_name='value')
                                #print(dmelt)
                                #print(dmelt)
                                lc = alt.Chart(dmelt).mark_bar().encode(
                                        x='date', y='value', color='flood', tooltip=['date','value'])
