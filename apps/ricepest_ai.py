...
from apps.data_operation import save_ann_model_to_postgis, load_all_ann_model_from_postgis, update_ann_model_to_postgis, update_dataset_title_ann_model_to_postgis, load_dataset_list, load_bph_model_data, update_ann_model_name_to_postgis

from apps.ricepest_train import bph_train_model, bph_predict_model
                ...
                with st.form("Deep Learning with LSTM Configuration", clear_on_submit = True):
                    ...
                    if submit_configuration_button:
                        if text_input_ann_name:
                            save_ann_model_to_postgis(text_input_ann_name, number_of_layers, number_of_neurons, optimizer, epoch)
                        else:
                ...
                with st.form(select_model, clear_on_submit = False):
                    ...
                    if update_configuration_button:
                        if text_update_ann_name:
                            update_ann_model_to_postgis(model['id'],text_update_ann_name,  epoch)
                        else:
                            st.error("Model title cannot be empty!")
                ...
                with st.expander("Train the model"):
                    ...
                    if button_train_model:
                        ...
                        with st.spinner("Training model... this can take very long to finish."):
                            history, model_name = bph_train_model(training_set, validation_set, model['n_epoch'])
                            print(history)
                    ...
                    with st.form("Select Input Date Range for Prediction"):
                        ...
                        if predict_button:                        
                            with st.spinner("Predicting... this might not takelong. It depends on size of data to be predicted."):
                                ...
                                predict_bph = bph_predict_model(point_df, model['model_name'])     
                                #print(prediction_date.tolist()) 
                                predict_bph['date'] = prediction_date.tolist()
                                #print(predict_bph.head())                          
                                
                                chart_data = predict_bph[pd.DatetimeIndex(predict_bph['date']).year == yr][['date','bph label','bph forecast']]  
