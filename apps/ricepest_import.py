...
class RicePestImportData(HydraHeadApp):
    ...
    def run(self):
        ...
        with st.sidebar:
            st.header('Rice Pest Outbreak')
            st.subheader('Import Data')

            uploaded_bph_file = st.file_uploader("Choose a CSV file to import all BPH-related data", type=['csv'])

            selected_data_layer = []

            if uploaded_bph_file is not None:   
                #if uploaded_file is not None:
                # Can be used wherever a "file-like" object isaccepted:
                with st.spinner("Loading data from file..."):
                    bph_df, bph_gdf = read_data_from_uploaded_file(uploaded_bph_file)
                    #print(bph_gdf.head())
                    ...
                    bph_year_options = st.multiselect("Select year of data to import", bph_years_list.values, bph_years_list.values[0])   
                if bph_year_options:
                    if set(['date', 'latitude', 'longitude']).issubset(bph_df.columns):      
                        fn_size = len(uploaded_bph_file.name)         
                        bph_dataset_title = st.text_input("Dataset title", value=uploaded_bph_file.name[:fn_size-4])                  
                        process_bph_data = st.button("Process & Upload Data")
                        #print(process_bph_data)
                        if process_bph_data:
                            if bph_dataset_title:
                                bph_gdf['dataset_title'] = bph_dataset_title
                                with st.spinner("Importing data to database..."):
                                    save_to_postgis(bph_gdf, "data_model_input")
        ...
        if selected_data_layer:
            ...
            if uploaded_bph_file is not None and len(bph_year_options) > 0:
                ...
                for yr in bph_year_options:
                    chart_data = selected_layer_df[pd.DatetimeIndex(selected_layer_df['date']).year == yr][['date','bph']]
                    #print(chart_data)
