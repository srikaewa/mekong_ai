...
from apps.data_operation import save_flood_ann_model_to_postgis, load_all_flood_ann_model_from_postgis, update_ann_model_to_postgis, update_dataset_title_ann_model_to_postgis, load_dataset_list, load_flood_model_data, update_ann_model_name_to_postgis

from apps.ricepest_train import flood_train_model, flood_predict_model



class FloodAIDashboard(HydraHeadApp):
    ...
    def run(self):
        ...
        print(self.title)
        st.subheader("Rice flood Deep Neural Network with LSTM Builder")

        model_df = load_all_flood_ann_model_from_postgis()


        row1_1, row1_2 = st.columns((2,1))
        with row1_1:
            st.markdown("**Deep Neural Network with LSTM**")
            st.markdown("This is neural network builder for modeling flood natural disaster. Create new builder and start import input and target data to build a model. The neural network with LSTM can be configured to achieve different performance. Once data is all ready, train the model and use the trained model to make prediction. This process is computationally expensive. When the model is ready, head to flood Dashboard section to view the result of prediction")
        ...
        with row1_2:
            with st.expander("Create Neural Network Model with LSTM"):
                ...
                with st.form("Deep Learning with LSTM Configuration", clear_on_submit = True):
                    text_input_ann_name = st.text_input("Enter Model Title", placeholder="flood Model #1")
                    ...
                    if submit_configuration_button:
                        if text_input_ann_name:
                            save_flood_ann_model_to_postgis(text_input_ann_name, number_of_layers, number_of_neurons, optimizer, epoch, number_of_days_in, number_of_days_out)
                        else:
        ...
        if select_model != '-':
            dataset_list = load_dataset_list("data_flood_model_input")
            st.markdown("---")            
                ...
                with st.form(select_model, clear_on_submit = False):
                    ...
                    if update_configuration_button:
                        if text_update_ann_name:
                            update_ann_model_to_postgis("flood_ann_model", model['id'],text_update_ann_name,  epoch)
                        else:
                            st.error("Model title cannot be empty!")
            ...
            with row2_2:
                with st.expander("Input Data Frame", expanded=False):
                    ...
                    if selected_dataset:
                        ...
                        if submit_input_button:
                            ...
                            update_dataset_title_ann_model_to_postgis("flood_ann_model", model['id'], selected_dataset)
                            st.success("Input added into model.")

                    if model['dataset_title'] is not None:
                        model_df = load_flood_model_data(model['dataset_title'])
                ...
                with st.expander("Train the model"):
                    ...
                    if model['dataset_title'] is not None:
                        ...
                        if button_train_model:
                            ...
                            #flood_column = model_df.pop('flood_lv')
                            ...
                            with st.spinner("Training flood ANN model... this can take very long to finish."):
                                history, model_name = flood_train_model(training_set, validation_set, model['n_epoch'], model['n_day'], model['n_out'])
                                #print(history)
                                update_ann_model_name_to_postgis("flood_ann_model", model['id'], model_name)
                ...
                with st.expander("Predict flood"):
                    ...
                    if model['dataset_title'] is not None:
                        with st.form("Select Input Date Range for Prediction"):
                            ...
                            flood_column = model_df.pop('flood')
                            model_df.insert(len(model_df.columns), 'flood', flood_column)
                            ...
                            if predict_button:                        
                                with st.spinner("Predicting... this might not takelong. It depends on size of data to be predicted."):
                                    ...
                                    predict_flood = flood_predict_model(point_df, model['model_name'], model['n_day'], model['n_out'])     
                                    #print(prediction_date.tolist()) 
                                    #predict_flood = predict_flood.iloc[:,1:]
                                    ...
                                    #print(prediction_date.tolist())
                                    #predict_flood['date'] = predict_flood.insert(loc=0, column='date', value=prediction_date)
                                    #predict_flood['date'] = predict_flood['date'].replace([1,2,3,4,5])

                                    

                                    """
                                    predict_flood.drop(columns = predict_flood.columns[0], axis = 1, inplace= True)
                                    print(predict_flood)
                                    print(predict_flood.shape)
                                    predict_flood['date'] = ['2000-01','2000-02','2000-03','2000-04','2000-05','2000-06','2000-07','2000-08','2000-09','2000-10','2000-11','2000-12']
                                    print(predict_flood)    
                                    ...
                                    chart_data = predict_flood[['date','flood label','flood forecast']]
                                    print(chart_data) 
                                    ...
                                    #lc = alt.Chart(chart_data).mark_bar().encode(x='date', y='flood_forecast', color='flood_forecast')
                                    ...
                                    #print(prediction_date.tolist()) 
                                    print(prediction_date)
                                    predict_flood['date'] = prediction_date.tolist()
                                    print(predict_flood.head())                          
                                    
                                    chart_data = predict_flood[pd.DatetimeIndex(predict_flood['date']).year == yr][['date','flood label','flood forecast']]  
                                    print(chart_data)
                                    selection = alt.selection_multi(fields=['forecast_data'], bind='legend')                  
                                    lc = alt.Chart(chart_data.melt('date', var_name='forecast_data', value_name='value')).mark_bar().encode(x='date', y='value', color='forecast_data',opacity=alt.condition(selection, alt.value(1), alt.value(0.2))).add_selection(selection)
                                    st.altair_chart(lc, use_container_width=True)
