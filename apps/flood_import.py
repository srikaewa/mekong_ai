...
class FloodImportData(HydraHeadApp):
    ...
    def run(self):
        ...
        with st.sidebar:
            st.header('Flood - Natural Disaster')
            st.subheader('Import Data')

            uploaded_flood_file = st.file_uploader("Choose a CSV file to import all flood-related data", type=['csv'])
            ...
            if uploaded_flood_file is not None:   
                ...
                with st.spinner("Loading data from file..."):
                    flood_df, flood_gdf = read_data_from_uploaded_file(uploaded_flood_file)
                    #print(flood_gdf.head())
                    
                    selected_data_layer = st.multiselect("Select data layer to view",['Flood Area', 'Weather'])
                    flood_years_list = pd.DatetimeIndex(flood_df['date']).year.unique()
                    flood_year_options = st.multiselect("Select year of data to import", flood_years_list.values, flood_years_list.values[0])   
                if flood_year_options:
                    if set(['date', 'address', 'latitude', 'longitude']).issubset(flood_df.columns):      
                        fn_size = len(uploaded_flood_file.name)         
                        flood_dataset_title = st.text_input("Dataset title", value=uploaded_flood_file.name[:fn_size-4])                  
                        process_flood_data = st.button("Process & Upload Data")
                        #print(process_flood_data)
                        if process_flood_data:
                            if flood_dataset_title:
                                flood_gdf['dataset_title'] = flood_dataset_title
                                with st.spinner("Importing data to database..."):
                                    save_to_postgis(flood_gdf, "data_flood_model_input")
        ...
        st.subheader("Import Flood Data")
        row1_1, row1_2 = st.columns((2,5))
        st.write("Import of flood raw data. Data can be easily uploaded with pre-defined format CSV file. Latitude & Longitude columns will be converted to geometry type of data and uploaded to database backend. Currently, data related to flood modeling mainly includes weather data (temperature, humidity, average soil surface moisture) and the information on flood (area and official annoncement of flood). Ascending time of data rows is also crucial to the time-series neural network.  To view specific data layer and year range, use multi select box from sidebar.")
        ...
        if selected_data_layer:
            ...
            for x in selected_data_layer:
                if x == 'Flood Area':
                    selected_columns.append('flood')
                if x == 'Weather':
                    selected_columns = selected_columns + ['humidity', 'max_precip', 'mean_precip',	'avg_ssm', 'dem']
            #print(selected_columns)
            if uploaded_flood_file is not None and len(flood_year_options) > 0:
                selected_layer_df = flood_df[selected_columns]
                buff_set = pd.DataFrame()
                for yr in flood_year_options:
                ...
                flood_all_df = grid_response['data']
                selected = grid_response['selected_rows'] 
                selected_df = pd.DataFrame(selected)
                for yr in flood_year_options:
                    chart_data = selected_layer_df[pd.DatetimeIndex(selected_layer_df['date']).year == yr][['date','flood']]
                    print(chart_data)
                    lc = alt.Chart(chart_data).mark_bar().encode(
                            x='date', y='flood', tooltip=['date','flood'])
