...
from apps.data_operation import save_to_postgis, load_from_postgis, load_ann_model_from_postgis, load_dataset_list, load_all_ann_model_from_postgis, load_blast_model_data


class RiceBlastDataLayers(HydraHeadApp):
    ...
    def run(self):
        ...
        def select_dataset_from_database(selected_dataset):
            return load_blast_model_data(selected_dataset)
        ...
        with st.sidebar:
            ...
            st.subheader('Rice Blast Outbreak')
            ...
            if selected_dataset != '-':
                blast_df = select_dataset_from_database(selected_dataset)
                #blast_df = blast_df.dropna(axis=1)
                selected_data_layer = st.multiselect("Select data layer to view",['Rice Blast','BUS','Weather'])
                blast_years_list = pd.DatetimeIndex(blast_df['date']).year.unique()
                blast_year_options = st.multiselect("Select year of data", blast_years_list.values, blast_years_list.values[0])  
 

        st.subheader("Rice Blast Monitoring")
        row1_1, row1_2 = st.columns((2,5))
        st.write("View time-series rice blast-related GIS data from database.")
            ...
            if selected_data_layer:
                ...
                for x in selected_data_layer:
                    if x == 'Rice Blast':
                        ...
                        chart_columns.append('blast_lv')
                    if x == 'BUS':
                        selected_columns.append('bus')
                        chart_columns.append('bus')
                    if x == 'Weather':
                        selected_columns = selected_columns + ['mint', 'maxt', 'temp', 'dew', 'humidity', 'precip']
                        chart_columns = chart_columns + ['mint', 'maxt', 'temp', 'dew', 'humidity', 'precip']
                    """
                ...
                if selected_data_layer != '-' and len(blast_year_options) > 0:
                ...
                with st.expander("View all blast & BUS data chart", expanded=False):
                    for yr in blast_year_options:
                        ...
                        #print(chart_data)
                        dmelt = chart_data.melt('date', var_name='blast vs BUS', value_name='value')
                        #print(dmelt)
                        lc = alt.Chart(dmelt).mark_bar().encode(
                                x='date', y='value', color='blast vs BUS', tooltip=['date','value'])
                        st.altair_chart(lc, use_container_width=True)
