...
class RiceBlastImportData(HydraHeadApp):
    ...
    def run(self):
        ...
        with st.sidebar:
            st.header('Rice Blast Outbreak')
            st.subheader('Import Data')

            uploaded_blast_file = st.file_uploader("Choose a CSV file to import all rice blast-related data", type=['csv'])
            ...
            if uploaded_blast_file is not None:   
                ...
                with st.spinner("Loading data from file..."):
                    blast_df, blast_gdf = read_data_from_uploaded_file(uploaded_blast_file)
                    #print(blast_gdf.head())
                    
                    selected_data_layer = st.multiselect("Select data layer to view",['Rice Blast', 'BUS', 'Weather','Rice Cultivated Area'])
                    blast_years_list = pd.DatetimeIndex(blast_df['date']).year.unique()
                    blast_year_options = st.multiselect("Select year of data to import", blast_years_list.values, blast_years_list.values[0])   
                if blast_year_options:
                    if set(['date', 'latitude', 'longitude']).issubset(blast_df.columns):      
                        fn_size = len(uploaded_blast_file.name)         
                        blast_dataset_title = st.text_input("Dataset title", value=uploaded_blast_file.name[:fn_size-4])                  
                        process_blast_data = st.button("Process & Upload Data")
                        #print(process_blast_data)
                        if process_blast_data:
                            if blast_dataset_title:
                                blast_gdf['dataset_title'] = blast_dataset_title
                                with st.spinner("Importing data to database..."):
                                    save_to_postgis(blast_gdf, "data_blast_model_input")
        ...
        st.subheader("Import Rice Blast Data")
        row1_1, row1_2 = st.columns((2,5))
        st.write("Import of rice blast-related raw data. Data can be easily uploaded with pre-defined format CSV file. Latitude & Longitude columns will be converted to geometry type of data and uploaded to database backend. Currently, data related to rice blast modeling includes level of rice blast to be modeled, BUS value, weather data and rice-variety in cultivated area. Ascending time of data rows is also crucial to the time-series neural network.  To view specific data layer and year range, use multi select box from sidebar.")
        ...
        if selected_data_layer:
            ...
            for x in selected_data_layer:
                if x == 'Rice Blast':
                    selected_columns.append('blast_lv')
                if x == 'BUS':
            ...
            if uploaded_blast_file is not None and len(blast_year_options) > 0:
                ...
                blast_all_df = grid_response['data']
                selected = grid_response['selected_rows'] 
                selected_df = pd.DataFrame(selected)
                for yr in blast_year_options:
                    chart_data = selected_layer_df[pd.DatetimeIndex(selected_layer_df['date']).year == yr][['date','blast_lv']]
                    print(chart_data)
                    lc = alt.Chart(chart_data).mark_bar().encode(
                            x='date', y='blast_lv', tooltip=['date','blast_lv'])
