...
def bph_train_model(frames_train, frames_validation, Epochs, n_day, n_out, n_layer, n_neuron):
    print(tf.version.VERSION)
    ...
    def create_model(model_funt):
        if model_funt == 5:
                model = tf.keras.models.Sequential([
                    keras.layers.LSTM(n_neuron, input_shape=(train_X.shape[1], train_X.shape[2]), return_sequences=True),
                    ...
                    keras.layers.BatchNormalization(),
                    keras.layers.Dropout(0.2),
                    keras.layers.LSTM(n_neuron, return_sequences=False),
                    keras.layers.BatchNormalization(),
                    keras.layers.Dropout(0.2),
                ...
                model = tf.keras.models.Sequential([
                    ...
                    keras.layers.LSTM(n_neuron, input_shape=(train_X.shape[1], train_X.shape[2]),return_sequences=True,activation='relu'),
                    keras.layers.BatchNormalization(),
                    keras.layers.Dropout(0.2),
                    keras.layers.LSTM(n_neuron, return_sequences=True, activation='relu'),
                    keras.layers.BatchNormalization(),
                    keras.layers.Dropout(0.2),
                    keras.layers.LSTM(n_neuron, activation='relu'),
                    keras.layers.BatchNormalization(),
                    keras.layers.Dropout(0.2),
                ...
                model = tf.keras.models.Sequential([
                    # Shape [batch, time, features] => [batch, time, lstm_units]
                    keras.layers.LSTM(n_neuron, input_shape=(train_X.shape[1], train_X.shape[2]),activation='relu'),
                    keras.layers.Dense(units=1)
                ])
    ...
    #------------------------------------------------------------------------------------------------------------------------------------------#
    training_mode = "newtrain"

    if training_mode == "newtrain":

        # Create a basic model instance
        model = create_model(n_layer)
    ...
    return history, model_name, acc, var_loss

def bph_retrain_model(frames_train, frames_validation, Epochs, model_file, n_day, n_out, n_layer, n_neuron):
    print(tf.version.VERSION)
    ...
    #------------------------------------------------------------------------------------------------------------------------------------------#
    #Export_folder_name = model_file + "/../"
    Export_folder_name = os.path.abspath(os.path.join(os.path.dirname(model_file), '.'))
    ...
    return history.history, acc, var_loss

def bph_train_model2(df, Epochs):
        ###########################################################################
...
def bph_predict_model(frames_predict, model_file, n_day, n_out):
    ...
    if True:
        ...
        df_out['bph_label'] = df['bph']
        df_out['bph_forecast'] = 0
        #print(df_out.head())
        #print(df_out.tail())
...
def drought_train_model(frames_train, frames_validation, Epochs, n_day, n_out):
    #print(tf.version.VERSION)
    ...
    Export_folder_name = "./Export_lstm_FLOOD/"
...
def drought_predict_model(frames_predict, model_file, n_day, n_out):
    #print(frames_predict)
    frames_predict = frames_predict.fillna(0)
    ...
    if True:
        ...
        df_out['latitude'] = df['latitude']
        df_out['longitude'] = df['longitude']
        df_out['drought_label'] = df['area_drought'].astype(int)
        df_out['drought_forecast'] = 0
        #print(df_out)
        #print(df_out.head())
        #print(df_out.tail())
        #print(df_out.iloc[:]['bph forecast'])
        ...
        print(forecast_data)
        # df_out.iloc[14:,5]  = forecast_data.astype(int)    #for lag 1
        df_out.iloc[(n_day+n_out-1):,4]  = forecast_data.astype(int)    #for lag 3
        # df_out.iloc[18:,5]  = forecast_data.astype(int)    #for lag 5
        # df_out.iloc[20:,5]  = forecast_data.astype(int)    #for lag 7
        print(df_out.head)
    #data_rmse = {'st_name': list_st,'rmse': list_mse}
...
def flood_train_model(frames_train, frames_validation, Epochs, n_day, n_out):
    print(tf.version.VERSION)
...
def flood_predict_model(frames_predict, model_file, n_day, n_out):
    frames_predict = frames_predict.fillna(0)
    ...
    if True:
        ...
        df_out['latitude'] = df['latitude']
        df_out['longitude'] = df['longitude']
        df_out['flood label'] = df['flood']
        df_out['flood forecast'] = 0
        #print(df_out.head())
        #print(df_out.tail())
        #print(df_out.iloc[:]['bph forecast'])
        #print(forecast_data)
